...
1. **Direct (O)**: If one attribute improves, the other improves too.
2. **Inverse (X)**: If one attribute improves, the other worsens.
3. **Neutral (—)**: The attributes are independent of each other.

### Examples:

| Relationship                     | Description |
|-----------------------------------|-------------|
| **Integrity vs. Efficiency**      | Inverse (X): More control on data access (integrity) reduces efficiency due to more code. |
| **Usability vs. Efficiency**      | Inverse (X): Improving user interface (usability) requires more code, reducing efficiency. |
| **Maintainability and Testability vs. Efficiency** | Inverse (X): Well-commented code (for maintainability) reduces efficiency. |
| **Flexibility and Reusability vs. Integrity** | Inverse (X): Flexible data structures can create security risks. |
| **Flexibility and Reusability vs. Maintainability** | Direct (O): Well-structured code is easier to maintain and reuse. |
| **Portability vs. Reusability**   | Direct (O): Portable code is often well-structured and reusable. |
| **Correctness vs. Efficiency**    | Neutral (—): Correct code can be either efficient or inefficient. |
...
| **Efficiency (E)**    |   |  | **E**  |   |   |   |   |   |   |   |   |
| **Integrity (I)**     |   |   | X | **I**  |   |   |   |  | |   |   |
| **Usability (U)**     | O  | O  | X | O  |  **U**  |   |   |   |   |   |   |
| **Maintainability (M)** | O  |  O | X |   |  O | **M**  |   |  |   |   |   |
| **Testability (T)**   |  O | O  | X |   |  O |O  |**T**   |  |   |   |   |
| **Flexibility (F)**   |  O |  O |  X |  |  O | O | O |  **F**  |   |   |   |
| **Portability (P)**   |   |   | X  |   |   | O  |  O |   | **P**  |   |   |
| **Reusability (R)**   |   |  X | X  |  X |   | O | O |  O | O | **R**  |   |
| **Interoperability (I)** |   |   | X  | X  |   |   |   |   | O  |   | **I**   |

*Figure. Perry’s relational model of software quality. (O - Direct, X - Inverse)*
